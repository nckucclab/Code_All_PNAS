...
* Submit to PNAS
*
* Stata Code "Table3_pop.do" is used to generate Table A.4 in Appendix D of Supporting Information A.  
*
************************************************************
...
*    xtabond `v' dln_p year_c*, endo($dy, lag(0,.)) inst(l(1/3).($dy)) maxldep(3) maxlags(3) pre($dy, lag(1,.))
*    eststo abond_`v'_al
*}
*esttab abond_* using "TableA5_D_SI_A.csv", title(`esttitle') mtitle replace b(3) se(3) constant scalars(N r2_w r2_o r2_b p chi2 sigma_u sigma_e rho) star(+ 0.10 * 0.05 ** 0.01 *** 0.001 ) wide

eststo clear
    xtreg dx2211 l.dx2211 $dy l.($dy) year_c*, fe
	eststo abond_hl_`v'_0
foreach v of global dz {
    xtabond `v' dln_p l_dln_p year_c*, endo($dy, lag(0,.)) inst(l(1/3).($dy)) maxldep(3) maxlags(3) pre($dy, lag(1,.))
    eststo abond_`v'_al
}
esttab abond_* using "TableA4_D_SI_A.csv", title(`esttitle') mtitle replace b(3) se(3) constant scalars(N r2_w r2_o r2_b p chi2 sigma_u sigma_e rho) star(+ 0.10 * 0.05 ** 0.01 *** 0.001 ) wide
...
*eststo clear
*    xtreg dx2211 l.dx2211 ldx2211_l $dy_l $dy_h l.($dy_l) l.($dy_h) dln_p_h dln_p_l year_c* l_year_c* TT_quart_l, fe
*	eststo abond_hl_`v'_0	
*foreach v of global dz {
*    xtabond `v' dln_p_h dln_p_l year_c* l_year_c* TT_quart_l, endo(l`v'_l, lag(0,1)) endo($dy_hl, lag(0,.)) inst(l(1/3).($dy_hl) l(2/3).(`v'_l)) maxldep(3) maxlags(3) pre(($dy_hl), lag(1,.))
*    eststo abond_hl_`v'_2g
*}
*esttab abond_* using "Table3_HL_PR_1_240907.csv.csv", title(`esttitle') mtitle replace b(3) se(3) constant scalars(N r2_w r2_o r2_b p chi2 sigma_u sigma_e rho) star(+ 0.10 * 0.05 ** 0.01 *** 0.001 ) wide

eststo clear
    xtreg dx2211 l.dx2211 ldx2211_l $dy_l $dy_h l.($dy_l) l.($dy_h) dln_p_h dln_p_l l_dln_p_h l_dln_p_l year_c* l_year_c* TT_quart_l, fe
	eststo abond_hl_`v'_0	
foreach v of global dz {
    xtabond `v' dln_p_h dln_p_l l_dln_p_h l_dln_p_l year_c* l_year_c*, endo(l`v'_l, lag(0,1)) endo($dy_hl, lag(0,.)) inst(l(1/3).($dy_hl) l(2/3).(`v'_l)) maxldep(3) maxlags(3) pre(($dy_hl), lag(1,.))
    eststo abond_hl_`v'_2g
}
esttab abond_* using "TableA4_HL_PR_DSIA.csv", title(`esttitle') mtitle replace b(3) se(3) constant scalars(N r2_w r2_o r2_b p chi2 sigma_u sigma_e rho) star(+ 0.10 * 0.05 ** 0.01 *** 0.001 ) wide
...
*    eststo abond_hl_`v'_2g
*}
*esttab abond_* using "Table3_HL_Total_1_240907.csv.csv", title(`esttitle') mtitle replace b(3) se(3) constant scalars(N r2_w r2_o r2_b p chi2 sigma_u sigma_e rho) star(+ 0.10 * 0.05 ** 0.01 *** 0.001 ) wide
...
esttab abond_* using "TableA4_HL_Tot_DSIA.csv", title(`esttitle') mtitle replace b(3) se(3) constant scalars(N r2_w r2_o r2_b p chi2 sigma_u sigma_e rho) star(+ 0.10 * 0.05 ** 0.01 *** 0.001 ) wide
...
*    eststo abond_hl_`v'_2g
*}
*esttab abond_* using "Table3_HL_EIU_1_240907.csv.csv", title(`esttitle') mtitle replace b(3) se(3) constant scalars(N r2_w r2_o r2_b p chi2 sigma_u sigma_e rho) star(+ 0.10 * 0.05 ** 0.01 *** 0.001 ) wide
...
esttab abond_* using "TableA4_HL_EIU_DSIA.csv", title(`esttitle') mtitle replace b(3) se(3) constant scalars(N r2_w r2_o r2_b p chi2 sigma_u sigma_e rho) star(+ 0.10 * 0.05 ** 0.01 *** 0.001 ) wide
